---
title: 我做了个 macOS 想法收件箱：ThoughtStream
...
1. **捕获阶段**：只管记，不管整理
2. **回顾阶段**：用 CLI 查询、过滤、导出，也可以交给 AI 做总结
...
**capture 的时候零摩擦，review 的时候交给 AI。** 整个过程不需要你打开任何一个笔记软件。
...
这个 CLI 是为脚本和 AI agent 准备的——你可以用它把 thoughts 喂给大模型做周总结、主题聚类、待办提取。
...
如果你也是那种天天有很多念头闪过的类型，试试看——也许你会发现，少想「怎么记」之后，反而记得更多了。
